...
// HMAC-SHA256 서명 생성 (쿠팡 WING API 인증)
...
app.post('/api/coupang/test', authenticate, async (req, res) => {
  try {
    ...
    if (!vendorId || !accessKey || !secretKey) {
      return res.status(400).json({
        success: false,
        message: 'vendorId, accessKey, secretKey are required',
      });
    }
...
app.post('/api/coupang/sync', authenticate, async (req, res) => {
  try {
    ...
    if (!vendorId || !accessKey || !secretKey || !startDate || !endDate) {
      return res.status(400).json({
        success: false,
        message: 'vendorId, accessKey, secretKey, startDate, endDate are required',
      });
    }
