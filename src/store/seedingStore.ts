...
// ========== 오늘 시딩 통계 타입 ==========
export interface TodaySeedingStats {
  listup: number;       // 오늘 리스트업된 수
  dmSent: number;       // 오늘 DM 발송한 수
  responded: number;    // 오늘 응답받은 수
  accepted: number;     // 오늘 수락된 수
  rejected: number;     // 오늘 거절된 수
}
...
interface SeedingStore {
  ...
  getTodayStats: (brand: Brand) => TodaySeedingStats;
...
function dbToInfluencer(record: any): SeedingInfluencer {
  return {
    ...
    shipping_cost: record.shipping_cost != null ? parseFloat(record.shipping_cost) : 0,
    status: record.status,
    shipping: {
      ...(record.shipping || {}),
    ...
    (set, get) => ({
      ...
      addInfluencersBulk: async (influencers) => {
        ...
        const toRecord = (inf: typeof influencers[0], index: number) => {
          ...
          if (index === 0) {
            console.log('[addInfluencersBulk] First input:', JSON.stringify({
              ...
              shipped_at: (inf as any).shipped_at,
              'shipping.shipped_at': inf.shipping?.shipped_at,
            }));
        ...
        console.log('[DEBUG] Before DB insert:', {
          ...
          shipped_at: (influencers[0] as any)?.shipped_at,
