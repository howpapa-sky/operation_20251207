...
**모든 코드 수정 작업 전에 반드시 `ERROR_PREVENTION_GUIDE.md`를 읽어야 합니다.**

이 가이드에는 자주 발생하는 오류 패턴, 금지사항, 체크리스트가 포함되어 있습니다.
가이드를 읽지 않고 작업하여 발생한 오류는 개발자 책임입니다.

필수 확인 항목:
- Falsy 값 처리 (|| vs ??) 규칙
- 절대 변경 금지 코드 (네이버 API URL, 토큰 공식, 인증 헤더)
- API 에러 처리 시 handleApiError() 사용
- 커밋 전 빌드/린트 체크리스트
